/* exptree.c - expression tree routines for swancc
 *
...
static struct nodestruct etree[ETREESIZE];    /* expression tree */
static struct nodestruct *ettop;

static void badlvalue(struct nodestruct *nodeptr);
...
static void needspv(struct nodestruct *nodeptr);
static struct typestruct *nodetype(struct nodestruct *nodeptr);
static int redscalar(struct nodestruct *nodeptr);
static struct nodestruct *unconvert(struct nodestruct *nodeptr);
    ...
    if ((bothscalar = lscalar | rscalar) & RSCALAR)
    {
        ...
        if (!(rscalar & DOUBLE))
        {
            nodeptr->right = castnode(dtype, right);
             ...
             ((nodeptr->tag == ANDOP && (redscalar(nodeptr->left.nodeptr) | redscalar(right)) & CHAR) ||
              ((nodeptr->tag == EOROP || nodeptr->tag == OROP) &&
               redscalar(nodeptr->left.nodeptr) & redscalar(right) & CHAR) ||
              (nodeptr->tag == MODOP && right->tag == LEAF && right->left.symptr->storage == CONSTANT &&
               (divisor = right->left.symptr->offset.offv, (uvalue_t)divisor <= MAXUCHTO + 1) &&
...
struct nodestruct *node(op_t t, struct nodestruct *p1, struct nodestruct *p2)
{
    ...
    switch ((op_t)t)
    {
        ...
        case CONDOP:
            ...
            {
                p2 = node(COLONOP, node(ADDRESSOP, p2->left.nodeptr, NULLNODE), node(ADDRESSOP, p2->right, NULLNODE));
    ...
    switch ((op_t)t)
    {
        ...
        case CASTOP:
            ...
            {            /* POINTER */
                neednonstruct(p1);
            }    /* H & S say functions & arrays must match */
            ...
            else if (target->indcount == 1 && targszdelta > 0 && target->flags != REGVAR)
            {
...
# endif
    ...
    if (target->storage == CONSTANT)
    {
        ...
        switch ((op_t)t)
        {
            case COMMAOP:
                return p2;
            case CONDOP:
                if ((lscalar & RSCALAR && dtargval != FALSE) || (!(lscalar & RSCALAR) && targval != FALSE))
                {
    ...
    switch ((op_t)t)
    {
        ...
        case ROOTLISTOP:
            goto node1;
        case PTRADDOP:
            if (p2->tag == LEAF && (source = p2->left.symptr)->storage == CONSTANT && source->type->scalar & ISCALAR)
            {
    ...
    switch ((op_t)t)
    {
        ...
        case PTRSUBOP:
            ...
            {
                targval = -((uvalue_t)(sourceval - targval) / target->type->nexttype->typesize);
            }
            else
            {
                targval = (uvalue_t)(targval - sourceval) / target->type->nexttype->typesize;
            }
            uflag = FALSE;
            break;
        case SLOP:
...
node1:
    {
        ...
        else
        {
            ...
            if ((nodeptr->weight = p1->weight) <= MAXREGS &&
                (regp2 == NULL || (nodeptr->weight = regp2->weight) < MAXREGS))
            {
...
static struct typestruct *nodetype(struct nodestruct *nodeptr)
{
    ...
    switch (nodeptr->tag)
    {
        ...
        case COLONOP:
            ...
            {
                targtype = (targtype->scalar | right->nodetype->scalar) & UNSIGNED ? uctype : ctype;
                ...
                (targtype->scalar & FLOAT) == 0)
            {
                ...
                {
                    return targtype;
                }  /* XXX - reduced from promote(targtype) */
            }

            if ((lscalar = targtype->scalar) != 0 && (rscalar = right->nodetype->scalar) != 0)
            {
            ...
            if (targtype->constructor & POINTER && (isconst0(right) || (right->nodetype->constructor & POINTER &&
                                                                        right->nodetype->nexttype->constructor & VOID)))
            {
                return targtype;
            }
            if (right->nodetype->constructor & POINTER &&
                (isconst0(left) || (targtype->constructor & POINTER && targtype->nexttype->constructor & VOID)))
            {
                return right->nodetype;
            }
        ...
        case ASSIGNOP:
            ...
            if ((targtype->constructor | right->nodetype->constructor) & STRUCTU && targtype != right->nodetype)
            {
...
static int redscalar(register struct nodestruct *nodeptr)
{
...
static struct nodestruct *unconvert(struct nodestruct *nodeptr)
{
    ...
    {
        return castnode(
            pointype(nodeptr->nodetype->constructor & ARRAY ? nodeptr->nodetype->nexttype : nodeptr->nodetype),
            nodeptr);
    }
    return nodeptr;
}

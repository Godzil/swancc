/* declare.c - declaration routines for swancc
 *
...
#include <swancc/input.h>        /* just for orig_cppmode */
...
static char gvar2name[2 + NAMESIZE];    /* space for structure keys and .. */

#define gvarname (gvar2name + 2)    /* last identifier declared */
static struct typestruct *gvartype;    /* type of last identifier declared */
static bool_t initlistflag;    /* remembers whether initializer is a list */

static struct typestruct *chainprefix(struct typestruct *pretype, struct typestruct *sufftype);
static void declaf(void);
static void declarator(void);
static void declarg(void);
...
static void inititem(struct typestruct *type);
static void initstruct(struct typestruct *type);
...
static struct typestruct *chainprefix(struct typestruct *pretype, struct typestruct *sufftype)
{
    if (pretype->nexttype != NULL)
    {
        ...
        {
            return prefix(ARRAY, pretype->typesize / pretype->nexttype->typesize * sufftype->typesize, sufftype);
        }
    }
    return sufftype;
}
    ...
    if (sym == LPAREN)        /* function */
    {
        ...
        if (declspec())
        {
            ...
            do
            {
                ...
                outbyte(' ');
#if 0
                if (gvarname[0] != 0 && gsymptr != NULL && gsymptr->level == level)
                {
...
#endif /* TESTING_PROTOTYPES */
        {
            while (sym == IDENT)
            {
                ...
                {
                    addloc(gsname, itype);
                }  /* adjust types and offsets later */
                nextsym();
                if (sym != COMMA)
                {
                    break;
...
{
    rdeclarator();
    if (gvartype->constructor == STRUCTU && gvartype->typesize == 0 && gvarsc != TYPEDEFDECL && gvarsc != EXTERNDECL)
    {
        error("undefined structure");
    }
}
...
static void declselt(struct typestruct *structype, offset_T *psoffset, struct typelist **ptypelist)
{
    ...
    while (sym != SEMICOLON && sym != EOFSYM)
    {
        ...
        else
        {
            ...
            {
                if (fieldwidth <= CHBITSTO)
                {    /* XXX! */
                    gvartype = ctype;
                }
                else if (fieldwidth <= INT32BITSTO)
                {    /* XXX */
                    gvartype = itype;
                }
                else
            ...
            {
                (gvarsymptr = addlorg(gvar2name, gvartype))->storage = NOSTORAGE;
        ...
        switch (sym)
        {
            ...
            case TYPEDECL:
                if (ntype == 1)
                {
                    ...
                    if (gvartype == itype && (gsymptr->type == stype || gsymptr->type == ltype))
                    {
                        ntype = 0;
                    }

                    /* allow int short and int long, blech */
                    if (gsymptr->type == itype && (gvartype == stype || gvartype == ltype))
                    {
                        nextsym();
                        break;
                    }
...
static struct typestruct *declsu()
{
    sym_t ogvarsc;
    offset_T soffset;
    ...
    structype->typesize = (struclength + ~structype->alignmask) & structype->alignmask;
    rbrace();
    gvarsc = ogvarsc;
    return structype;
}
...
static void declfunc()
{
    ...
    else if (gvarsymptr->type != gvartype || (gvarsymptr->flags & INITIALIZED))
    {
        multidecl(gvarname);
    }    /* different type or full declare */
    ...
    for (symptr = &locsyms[0] ; symptr < locptr && symptr->flags != STRUCTNAME &&
                                *symptr->name.namea >= 'A' ; symptr = (struct symstruct *)align(
...
static void idecllist()
{
    ...
    while (sym != SEMICOLON)
    {
        ...
        else if (gvarsymptr != NULL && (gvarsymptr->level == level || gvartype->constructor == FUNCTION ||
                                        (gvarsc == EXTERNDECL && gvarsymptr->level == GLBLEVEL)))
        {
            ...
            {
                if (gvartype->constructor != ARRAY || gvarsymptr->type->constructor != ARRAY ||
                    gvartype->nexttype != gvarsymptr->type->nexttype ||
                    (gvartype->typesize != 0 && gvarsymptr->type->typesize != 0))
                {
                    multidecl(gvarname);
                }
        ...
        else
        {
            ...
            if ((gvarsc != REGDECL || !regdecl()) && gvartype->constructor != FUNCTION && gvarsc != STATICDECL &&
                gvarsc != TYPEDEFDECL)
            {
                gvarsymptr->offset.offi = softsp = (softsp & gvartype->alignmask) - gvartype->typesize;
            }
        }
        if (gvarsc == STATICDECL)
        {
        ...
        if (sym == ASSIGNOP)
        {
            ...
            if (level == GLBLEVEL || gvarsc == STATICDECL)
            {
                ...
                newoffset = (dataoffset + ~inittype->alignmask) & inittype->alignmask;
        ...
        else if (level != GLBLEVEL && gvarsc == STATICDECL && gvartype->constructor != FUNCTION)
        {
#ifndef DIRECTPAGE
            bssseg();
#else
...
static void initarray(struct typestruct *type)
{
    ...
    else
    {
        ...
        {
            inititem(basetype);
        } while (--remaining != 0 && sym == COMMA && initlistflag && (nextsym(), sym != RBRACE));
    }
    if (dimension == 0)
    {
        if (type == gvartype)
...
static void initstruct(struct typestruct *type)
{
    ...
    if ((typelist = type->listtype) != NULL)
    {
        do
        {
            ...
            inititem(memtype);
        } while ((typelist = typelist->tlnext) != NULL && sym == COMMA && initlistflag && (nextsym(), sym != RBRACE));
    }
